function boxes = detect_fast_exemplar_half(im,model,thresh,filters_exemplar,exemplar_constant,fr_indx,calib_flag,resp_outdir,exemplar_indxs)
...
min_exemplar_score = -1.5192; max_exemplar_score = 1.4415; 
...
% change filters_exemplar from 26*H*W to 108*H*W for code compatibility
filters_exemplar_108 = {};
for i=1:numel(filters_exemplar)
    curr_cell = filters_exemplar{i};
    new_cell = {};
    for j=1:numel(curr_cell)
    ...
    filters_exemplar_108{i} = new_cell;
end;
  ...
  for c  = 1:length(model.components),
    ...
    for k = 1:numparts,
        ...
        if isempty(resp{level}),
            ...
            if calib_flag == 1
                resp_exemplar{level} = fconv(pyra.feat{level},filters_exemplar_108{max_exemplar_indx},1,length(filters_exemplar_108{max_exemplar_indx}));
            end;
