function test_video_half(cache_dir,mat_file, vid_dir, exemplar_constant, phase_number, calib_flag, ex_cnsnt_pick_arr, exemplar_flag, exemplar_indxs, weights_exemplar)
...
if exemplar_flag ~= 1
    ...
    for ph = 2:phase_number
        [exemplar_indxs, new_exemplar_indxs] = get_phasewise_exemplar_indxs_half(cache_dir,vid_dir,exemplar_indxs,ph,ex_cnsnt_pick_arr(ph-1));
    ...
    for i=1:numel(exemplar_indxs)
        tmp_ = get_exemplar_weights_half(cache_dir,vid_dir,i,phase_number,ex_cnsnt_pick_arr);
...
imlist = dir(fullfile(vid_in_dir,'*.png'));
...
for i = 1:length(imlist)
    ...
    if ~exist(fullfile(out_dir, [imlist(i).name(1:end-4) '_pose.mat']))
        ...
        if phase_number > 1
            ...
            boxes = detect_fast_exemplar_half(im, model, min(model.thresh,-1), curr_weights,exemplar_constant,i,calib_flag,resp_outdir,exemplar_indxs);            
