...
def create_pose_features_dataset(_features_dataset_name, _labels_dataset_name):
    ...
    for _image_name in _shot_0_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '0', _image_name)
        _idx += 1

    for _image_name in _shot_1_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '1', _image_name)
        _shot_labels[_idx] = 1
        _idx += 1

    for _image_name in _shot_2_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '2', _image_name)
        _shot_labels[_idx] = 2
        _idx += 1

    for _image_name in _shot_3_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '3', _image_name)
        _shot_labels[_idx] = 3
        _idx += 1

    for _image_name in _shot_4_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '4', _image_name)
        _shot_labels[_idx] = 4
        _idx += 1

    for _image_name in _shot_5_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '5', _image_name)
        _shot_labels[_idx] = 5
        _idx += 1

    for _image_name in _shot_6_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '6', _image_name)
        _shot_labels[_idx] = 6
        _idx += 1

    for _image_name in _shot_7_images_name:
        _feature_pose_mat[_idx] = full_pose_estimation(_eng, '7', _image_name)
        _shot_labels[_idx] = 7
        _idx += 1
