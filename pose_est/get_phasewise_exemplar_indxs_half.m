function [all_exemplar_indxs,topK_instances] = get_phasewise_exemplar_indxs_half(cache_dir,vid_dir,exemplar_indxs,phase_num,ex_cnsnt_val)
...
offbeat_feat = get_pruning_criteria_feats_binary_half(out_dir); % SVM initialization, binary features
...
if numel(new_nbds) > 1
    ...
    [~,t__] = sort(new_nbd_avg_onbeat,'descend');
    nbd_pruned = t__(1:2);
...
nbd_final_picked = new_nbds(nbd_pruned(new_nbd_sc_picked(b_(1))));

topK_instances = [];
for nbd_picked = nbd_final_picked
    % pick instances from nbd_picked
    %     curr_nbd_pick_arr = pick_arr(pick_arr >= (nbd_picked-1)*10+1 & pick_arr <= nbd_picked*10);
    %     if isempty(curr_nbd_pick_arr)        % if empty, proceed with just score
    ...
    % for later phases. first OFFBEAT -> OFFBEAT_Subtle -> score.
    pick_ = [];
...
% fprintf('NEW: %d %d : %.4f \n',numel(topK_instances),numel(nbd_final_picked),mean(score_train(topK_instances))/26);
% for i=1:numel(topK_instances)
%     fprintf('%d ',topK_instances(i));
% end;
% fprintf('\n');
